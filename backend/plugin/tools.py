...
def plugin_router_inject() -> None:
    ...
    :return:
    """
    plugins = get_plugins()  # 获取插件列表（只包含插件名）
    for plugin in plugins:
        toml_path = os.path.join(PLUGIN_DIR, plugin, "plugin.toml")  # 拼接插件路径和 plugin.toml 文件名
        if not os.path.exists(toml_path):  # 检查插件是否存在 plugin.toml 文件
            raise PluginInjectError(f"插件 {plugin} 缺少 plugin.toml 配置文件，请检查插件是否合法")

        # 获取 plugin.toml 配置
        with open(toml_path, "r", encoding="utf-8") as f:
            data = rtoml.load(f)  # 加载文件内容 -> {'app': {'include': 'admin'}, 'api': {'notice': {...}}}
        api = data.get("api", {})

        # 非独立 app
        if api:
            app_include = data.get("app", {}).get("include", "")  # 获取插件中 app 配置的 include
            if not app_include:  # 判断是否存在 include 配置
                raise PluginInjectError(f"非独立 app 插件 {plugin} 配置文件存在错误，请检查")

            # 插件中 API 路由文件的路径
            plugin_api_path = os.path.join(PLUGIN_DIR, plugin, "api")  # 插件中 API 路由文件的路径
            if not os.path.exists(plugin_api_path):  # 判断是否存在 api 目录
                raise PluginInjectError(f"插件 {plugin} 缺少 api 目录，请检查插件文件是否完整")

            # 将插件路由注入到对应模块的路由中
            for root, _, api_files in os.walk(
                plugin_api_path
            ):  # 解析出 api 目录下的所有文件，root 是当前目录，api_files 是当前目录下的所有文件
                for file in api_files:
                    if file.endswith(".py") and file != "__init__.py":
                        # 解析插件路由配置
                        prefix = (
                            data.get("api", {}).get(f"{file[:-3]}", {}).get("prefix", "")
                        )  # 获取插件路由配置中的前缀
                        tags = data.get("api", {}).get(f"{file[:-3]}", {}).get("tags", [])  # 获取插件路由配置中的标签

                        # 获取插件路由模块
                        file_path = os.path.join(root, file)  # 插件路由模块的路径
                        path_to_module_str = os.path.relpath(file_path, PLUGIN_DIR).replace(os.sep, ".")[
                            :-3
                        ]  # 插件路由模块的路径（相对于插件目录）
                        module_path = (
                            f"backend.plugin.{path_to_module_str}"  # 插件路由模块的路径（相对于 backend.plugin）
                        )
                        try:
                            module = import_module_cached(module_path)  # 根据模块路径导入模块
                        except PluginInjectError as e:
                            raise PluginInjectError(f"导入非独立 app 插件 {plugin} 模块 {module_path} 失败：{e}") from e
                        plugin_router = getattr(module, "router", None)  # 获取插件路由模块中的 router
                        ...
                        relative_path = os.path.relpath(
                            root, plugin_api_path
                        )  # 插件路由模块的路径（相对于插件 api 目录）
                        # 源程序路由模块的路径（相对于 backend.app）
                        target_module_path = f"backend.app.{app_include}.api.{relative_path.replace(os.sep, '.')}"
                        try:
                            target_module = import_module_cached(target_module_path)  # 根据模块路径导入模块
                        except PluginInjectError as e:
                            raise PluginInjectError(f"导入源程序模块 {target_module_path} 失败：{e}") from e
                        target_router = getattr(target_module, "router", None)  # 获取源程序路由模块中的 router
